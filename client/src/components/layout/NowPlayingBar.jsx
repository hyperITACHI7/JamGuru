...
export default function NowPlayingBar() {
  ...
  async function toggleLike() {
    ...
    try {
      if (liked) {
        await unlikeSong(track.spotifyId)
        setLiked(false)
      } else {
        await likeSong(track.spotifyId)
        setLiked(true)
      }
    } catch (_) {}
  }
  ...
  return (
    <>
    <footer className="h-[90px] bg-[#181818] border-t border-white/5 flex items-center px-4 gap-4 flex-shrink-0">
