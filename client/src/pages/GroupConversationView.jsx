  ...
  return parts.map((part, i) => {
    ...
    return (
      <button key={i} onClick={() => cycleVar(key, ph.options)}
        className="inline-flex items-center gap-1 bg-purple-500/20 border border-purple-500/40 text-purple-400 text-xs font-semibold px-2 py-0.5 rounded-full hover:bg-purple-500/30 transition-colors">
              ...
              <div className="overflow-y-auto flex-1 px-3 pb-3 space-y-2">
                {REQUEST_TEMPLATES.map(tmpl => (
                  <button key={tmpl.id} onClick={() => selectTemplate(tmpl)}
                    ...
                    <p className="text-purple-400 text-[9px] font-bold uppercase tracking-wider mb-1">{tmpl.label}</p>
