...
export default function FriendsDmPanel({ selected, onSelect, className = '' }) {
  ...
  return (
    <div className={`w-[240px] flex-shrink-0 min-h-0 border-l border-white/10 shadow-[-6px_0_16px_-10px_rgba(0,0,0,0.7)] flex flex-col overflow-hidden ${className}`}>
      ...
      <div className="flex-shrink-0 px-2.5 py-2.5 md:py-2">
        <div className="relative">
          ...
          <input
            ...
            className="w-full bg-[#1a1a1a] text-white text-sm md:text-xs rounded-full pl-8 pr-8 py-2.5 md:py-1.5 focus:outline-none focus:ring-1 focus:ring-[#1DB954]/50 placeholder-[#535353]"
          />
          {query && (
            <button
              onClick={() => setQuery('')}
