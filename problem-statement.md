...
JamGuru is a **functional advanced prototype** — deployed on a live server with a real database, covering the full intended feature set, but held back from MVP classification by a single platform constraint: song playback is limited to 30-second Spotify preview clips rather than full tracks. Many tracks have no preview URL at all. Because playback is central to a music discovery experience, this limitation keeps the product in prototype territory despite the social layer being substantially complete.

### What Is Fully Implemented

**Authentication & Onboarding**
- Manual registration and login (username/password)
- Spotify OAuth (account creation + token refresh)
- New-user onboarding wizard: 5-step guided flow covering the app concept, adding a first friend, and setting up a taste profile — shown once on first login and never repeated

**Library**
- Spotify liked songs sync (one-tap, refreshable)
- Playlist import via URL (public and private playlists)
- Liked Songs page with per-track unlike action
- Library integrity invariant: disliking a song removes it from playlists; playlist presence overrides conflicting dislike records

**Recommendations — Sending**
- Send a song to one or multiple friends in a single action, with optional context
- Share panel with friend list, AI Predict ranking (friends sorted by taste compatibility with the song being shared)
- Already-explored sender hints: "Already has it", "Already passed", "Not their vibe" badges shown on candidate songs before sending

**Recommendations — Receiving**
- Like (adds to library, updates sender scores, triggers tag picker)
- Dismiss (soft-hides from inbox, reversible, no score impact)
- Dislike (negative taste signal, removes from inbox and library, no sender penalty)
- Tag feedback on likes: selectable chips describing why a song resonated, visible to the sender in the conversation thread
- Reconsider button: re-enables a previously dismissed or disliked card, including songs marked "Already in your library"

**Discovery Inbox**
- Time-grouped buckets (Today, Yesterday, This Week, Earlier)
- Default sort by recommendation score; toggle to chronological
- Pre-discovered filtering: songs already in the user's library are removed from the main inbox view and shown quietly in the conversation thread with an "Already in your library" label
- Prior-feedback treatment: previously dismissed/disliked songs are grayed out with "Not for me" / "Not my vibe" labels and a Reconsider button

**Conversation View**
- Per-friend threaded history of all shared songs, displayed as chat bubbles
- Sent and received bubbles with play, like, dismiss, dislike, and Reconsider actions
- Sender-side labels reflecting recipient feedback ("Not Arpit's vibe", "Not for Arpit")
- Reply-to-request threading: request bubbles linked to their fulfillment

**Song Requests**
- Six structured request templates with fill-in-the-blank tag pickers
- Reply picker with three sections: AI suggestions, user's library (AI-ranked by relevance), and Spotify search
- Group broadcast requests: any group member can fulfill

**JamGuru Scoring**
- Daily discovery score: likes received ÷ recommendations sent, updated in real time
- Monthly discovery score: sum of daily scores for the current month
- Monthly reset on the 1st of each month
- Personal trust ranking: fully private, per-user — no global leaderboard
- JamGuru title: awarded to the friend with the highest monthly discovery score for a given user
- "JamGuru for X listeners" count visible on user profile

**Groups**
- Group creation, member management, group feed
- Group recommendations (isolated from personal inbox)
- Group taste profile (manual or AI-generated from group history)
- Group scoring (normalised by group size)

**AI Features**
- Picked For You: 3 daily home-screen picks generated from the user's taste profile; randomised discovery angle per session; cached to avoid redundant calls
- AI Suggest in DMs: library-first pass filtered for recipient novelty, with external generation fallback; session-level deduplication
- AI Suggest in Groups: same approach using the group taste profile
- AI Predict: ranks friends by taste compatibility when sharing a song
- AI-powered request fulfillment: library ranking + external generation, merged in reply picker
- Provider-agnostic AI backend: supports Groq, OpenAI-compatible endpoints, or Anthropic Claude via environment variables

**Taste Profile**
- Auto-generated by AI from liked songs and playlists
- Manually editable (genres, moods, eras, artists)
- Pinned vs. AI-discovered tag distinction
- Group-level collective taste profile

**Infrastructure**
- Real-time notifications via Server-Sent Events (SSE)
- Web Push notifications for key events when away from the app
- Last.fm tag enrichment (asynchronous, per newly cached song)
- Deployed: Node/Express server + PostgreSQL on Render; React/Vite client served as static build

### What Is Missing for MVP

| Gap | Reason |
|---|---|
| Full song playback | Spotify Web Playback SDK requires Spotify Premium; preview URLs cap out at 30 seconds and are absent on many tracks |
| Deep-link to Spotify | "Open in Spotify" button on each song card — not yet implemented, but low-effort and high-value as an interim handoff |

The social discovery layer, scoring system, AI integration, and onboarding are sufficiently complete for real-world testing. Full-track playback is the remaining gap between the current advanced prototype and a shippable MVP.
